...
function calculateResults() {
  ...
  highlightBox.innerHTML = `
    ...
    <p>${getAnchorComment(max.anchor, scores[max.anchor], average)}</p>
  `;
  app.appendChild(highlightBox);
  resultLines.push(`Ancora dominante: ${max.anchor}`);

  const subTitle = document.createElement('h4');
  subTitle.textContent = 'Ancore sopra la media:';
  ...
  aboveAverage.forEach(anchor => {
    ...
    comment.innerHTML = `
      <p><strong>${anchor}</strong>: ${getAnchorComment(anchor, scores[anchor], average)}</p>
    `;
...
function getAnchorComment(anchor, score, avg) {
  ...
  return intensity + (base[anchor] || anchor);
}
